...
public abstract class RecordQueryDAO {
    ...
    private static final String RECORDS_WITH_LIMIT_SQL  = "SELECT " +
            ...
            "ORDER BY ck.key ASC LIMIT :limit OFFSET :offset";
    ...
    private static final String RECORDS_BY_CONTENT_SQL = "SELECT " +
            ...
            "  FROM  entry e " +
            "  GROUP BY  e.key " +
            " ) ck JOIN entry AS e ON " +
            " ck.entry_number = e.entry_number JOIN item AS i ON " +
            " e.sha256hex = i.sha256hex " +
            "WHERE  i.content @> :contentPGobject LIMIT 100; ";
